...
def measure(&block)
  ...
  if no_gc
    ...
    # and our own code before measurement
    GC.start
  end

  memory_before = 'ps -o rss = -p #{Process.pid'.to_i/1024
  
  gc_stat_before = GC.stat # returns number of GC runs
  
  time = Benchmark.realtime do
    yield # when a method expects a block, it invokes it by calling yield function
  end
